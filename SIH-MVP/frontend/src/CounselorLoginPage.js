...
const CounselorLoginPage = () => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            const response = await fetch('http://127.0.0.1:5000/api/counselor-login', {
                method: 'POST',
            ...
            if (response.ok && data.role === 'counselor') {
                login(data.role, username);
                navigate('/dashboard');
            } else {
                setError(data.message || 'Only counselor users can log in here. Please use the correct login page for your role.');
            }
        } catch (err) {
    ...
    return (
        <div className="flex items-center justify-center min-h-screen p-6 bg-gradient-to-br from-green-50 via-blue-50 to-purple-50">
            <div className="w-full max-w-sm p-10 space-y-8 bg-white/90 backdrop-blur-sm border border-white/40 rounded-3xl shadow-2xl transition-all duration-300">
                <h2 className="text-5xl font-extrabold text-center text-gray-900">Counselor Login</h2>
                <p className="text-center text-gray-600 text-lg">Sign in as a counselor to access the dashboard.</p>
